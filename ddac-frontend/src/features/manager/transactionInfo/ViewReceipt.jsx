...
import { getPaymentById } from "../../staff/services/paymentService";
import { getAppointmentById } from "../../staff/services/appointmentService";
...
export default function ViewReceipt() {
    ...
    const loadReceiptData = async () => {
        try {
            ...
            if (!validStatuses.includes(txnData.status.toLowerCase())) {
                setLoading(false);
                return;
            }
            ...
            if (txnData.receipt) {
                try {
                    receiptData = typeof txnData.receipt === 'string'
                        ? JSON.parse(txnData.receipt)
                        : txnData.receipt;
                } catch (parseError) {
                    console.error("Error parsing receipt JSON:", parseError);
                }
            }
            ...
            <div className="min-h-screen bg-gray-50 py-8">
                <div className="container mx-auto px-4 max-w-4xl">
                    ...
                    <div className="mb-6 flex gap-4 print:hidden">
                        <button
                            onClick={() => navigate(-1)}
                            className="cursor-pointer flex items-center gap-2 px-4 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700 transition-colors"
                        >
                            <FaArrowLeft /> Back
                        </button>
                        <button
