...
import { FaEye, FaCreditCard, FaDollarSign, FaCheckCircle, FaClock, FaFileInvoice } from "react-icons/fa";
...
export default function PaymentList() {
  ...
  const loadPaidTransactions = async () => {
    try {
      const allTransactions = await paymentService.getAllPayments();
      const paidTxns = (allTransactions || []).filter(
        (txn) => (txn.status || "").toLowerCase() === "paid"
      );
      setPaidTransactions(paidTxns);
    } catch (error) {
  ...
  const viewReceipt = (transaction) => {
    ...
    navigate(`/staff/payments/receipt/view?transactionId=${transactionId}`);
  };
      ...
      <div className="w-full max-w-full overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-card rounded-xl shadow-soft p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-heading text-2xl font-bold">{paidTransactions.length}</h3>
                <p className="text-muted text-sm">Paid Transactions</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 border-b border-color">
          <div className="flex gap-4">
            ...
            >
              Transaction History ({paidTransactions.length})
            </button>
          </div>
        </div>
          ...
          <div className="bg-card rounded-xl shadow-soft overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full min-w-full table-fixed">
                <thead className="bg-primary border-b border-color">
                  <tr>
                    <th className="text-left py-4 px-6 text-ondark font-semibold text-sm break-all">Transaction ID</th>
                    <th className="text-left py-4 px-6 text-ondark font-semibold text-sm break-all">Appointment ID</th>
                ...
                <tbody>
                  {paidTransactions.length > 0 ? (
                    paidTransactions.map((txn, index) => (
                      ...
                      >
                        <td className="py-4 px-6">
                          <p className="text-body text-sm break-all">{txn.id || txn._id || "N/A"}</p>
                        </td>
                        <td className="py-4 px-6">
                          <p className="text-body text-sm break-all">{txn.appointmentId || "N/A"}</p>
                        </td>
                        <td className="py-4 px-6">
                          <p className="text-body text-sm font-semibold">{formatCurrency(txn.amount)}</p>
                        </td>
                        <td className="py-4 px-6">
                          <p className="text-body text-sm">{txn.paymentMethod || "N/A"}</p>
                        </td>
                        <td className="py-4 px-6">
                          <p className="text-body text-sm">
                            {txn.paymentTime ? formatStaffDate(txn.paymentTime) : "N/A"}
                          </p>
                        </td>
                        <td className="py-4 px-6">
                    ...
                    <tr>
                      <td colSpan="7" className="py-12 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-muted text-lg">No transaction history</p>
                          <p className="text-muted text-sm">Paid transactions will appear here</p>
                        </div>
                      </td>
                    </tr>
                  )}
